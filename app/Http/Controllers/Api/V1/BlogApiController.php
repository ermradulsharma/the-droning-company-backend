...
class BlogApiController extends Controller
{
    ...
    public function homeFeatured(Request $request)
    {
        ...
        $category_4 = BlogCategory::query()
            ->promoted()
            ->skip(1)
            ->first();

        $category_2 = BlogCategory::where(['title' => 'Product News'])->first();

        //dd($category_4);

        if (!$category_4 && !$category_1) {
            return response()->json([
                'statusCode' => 404,
                'message' => 'featured Category not found!',
                'data' => []
        ...
        $category_1_blog = Blog::whereHas('blog_categories', function ($q) use ($category_1) {
            $q->where('blog_category_id', @$category_1->id);
        })->where('status', '1')->orderBy('id', 'DESC')->skip(1)->take(2)->get();
    ...
    public function getBlogsByCategory(Request $request)
    {
        ...
        $limit = (isset($request->limit) && $request->limit != 'undefined') ? $request->limit : 4;
        ...
        /*$category_ids = BlogCategory::whereIn('title', $categories)->pluck('id');
        $posts = Blog::whereHas('blog_categories', function ($q) use ($category_ids) {
            $q->whereIn('blog_category_id', $category_ids);
        })->latest('created_at')->skip($skip)->take($limit)->get();*/
    ...
    {
        $blogs = Blog::query()
            ->select('id', 'title', 'image', 'slug', 'excerpt', 'created_at')
            ->latest('id')
            ->take(50)
            ->get();
