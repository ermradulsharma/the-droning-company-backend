...
class CompanyVideoController extends Controller
{
    ...
    public function store(Request $request, int $user_id)
    {
        ...
        if ($request->has('reel_video')) {
            foreach ($request->input('reel_video') as $key => $value) {
                ...
                $video->company_id=$profile->id;
                $video->save();
                $ids[]=$video->id;
                if (ucfirst($value['position'])=="Main") {
                    CompanyVideo::where('company_id', $profile->id)->whereNotIn('id', [$video->id])->update(['position'=>'Gallery']);
                }
            }
        }
        $videos=CompanyVideo::whereIn('id', $ids)->select('id as pilot_video_id', 'type as video_type', 'video as video_url', 'video_key', 'position')->get();
        return response()->json([
    ...
    */
    public function show(Request $request, int $user_id)
    {
        ...
        $videos=CompanyVideo::where('company_id', $company_id)
    ...
    */
    ...
    {
        $video=CompanyVideo::find($pilot_video_id);
    ...
    */
    public function markAsMainVideo(Request $request, int $pilot_video_id)
    {
        ...
        CompanyVideo::where('pilot_profile_id', $video->pilot_profile_id)
                    ->whereNotIn('id', [$video->id])
                    ->update(['position'=>'Gallery']);


        $videos=CompanyVideo::where('pilot_profile_id', $video->pilot_profile_id)
                        ->select('id as pilot_video_id', 'type as video_type', 'video as video_url', 'video_key', 'position')
                        ->latest()
                        ->get();
