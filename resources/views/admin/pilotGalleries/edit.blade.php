...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pilot-galleries.update", [$pilotGallery->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="pilot_profile_id" value="{{$pilotGallery->pilot_profile_id}}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\PilotGallery::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $pilotGallery->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
