...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.blog-categories.update", [$blogCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.blogCategory.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\BlogCategory::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $blogCategory->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="meta_keyword">{{ trans('cruds.blogCategory.fields.meta_keyword') }}</label>
                <input class="form-control {{ $errors->has('meta_keyword') ? 'is-invalid' : '' }}" type="text" name="meta_keyword" id="meta_keyword" value="{{ old('meta_keyword', $blogCategory->meta_keyword) }}">
                @if($errors->has('meta_keyword'))
                    <div class="invalid-feedback">
                        {{ $errors->first('meta_keyword') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.blogCategory.fields.meta_keyword_helper') }}</span>
            </div>
            <div class="form-group">
