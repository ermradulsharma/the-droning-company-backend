    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.blogs.update", [$blog->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group card-data">
                <img src="{{$blog->image}}" height="auto" class="img-responsive" width="50%">
            </div>
             <div class="form-group card-data">
                <div class="custom-file">
                    ...
                    <label class="custom-file-label" for="customFile">Upload new features image</label>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('blog_categories') ? 'is-invalid' : '' }}" name="blog_categories[]" id="blog_categories" multiple>
                    @foreach($blog_categories as $id => $blog_category)
                        <option value="{{ $id }}" {{ (in_array($id, old('blog_categories', [])) || $blog->blog_categories->contains($id)) ? 'selected' : '' }}>{{ $blog_category }}</option>
                    @endforeach
                </select>
            ...
            type="checkbox" 
            value="1" {{$blog->is_featured==1 ? 'checked':''}}>
