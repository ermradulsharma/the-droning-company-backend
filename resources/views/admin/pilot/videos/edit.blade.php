...
<div class="card">
    ...
    @endif
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pilot_videos.update", $videos->id) }}" enctype="multipart/form-data" onsubmit="return checkValidation();">
            @method('PUT')
            @csrf
            <div class="form-group">
                <input class="form-control" type="hidden" name="id" value="{{ $videos->id }}" readonly>
            </div>
            
            <div class="form-group">
                <label class="required" for="travel_option">Video Type</label>                
                <div class="d-inline-block custom-control custom-radio mr-2">
                    <input type="radio" class="custom-control-input" name="type" id="video_type1" value="Youtube" {{ ($videos->type=="Youtube")? "checked" : "" }}  required>
                    <label class="custom-control-label" for="video_type1">You Tube</label>
                </div>
                <div class="d-inline-block custom-control custom-radio mr-2">
                    <input type="radio" class="custom-control-input" name="type" id="video_type2" value="Vimeo" {{ ($videos->type=="Vimeo")? "checked" : "" }} >
                    <label class="custom-control-label" for="video_type2">Vimeo</label>
                </div>
            </div>
            <div class="form-group">
                <label class="required" for="title">Enter Video Url</label>
                <input class="form-control {{ $errors->has('video') ? 'is-invalid' : '' }}" type="text" name="video" id="video" value="{{ old('video', $videos->video) }}" required>
                @if($errors->has('video'))
